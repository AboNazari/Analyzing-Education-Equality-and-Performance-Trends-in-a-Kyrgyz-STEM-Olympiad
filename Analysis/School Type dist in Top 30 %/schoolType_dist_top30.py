...
grouped = df_top_30.groupby(['class', 'schoolType']).size().reset_index(name='count')
...
for class_name in classes:
    ...
    plt.title(f"Score Distribution in Top 30% for School Type in Class: {class_name}")
    plt.ylabel('Number of Students')
    plt.tight_layout()
    plt.savefig(f"Top_30_Score_Distribution_{class_name}.png")
    plt.show()
...
plt.title("Score Distribution in Top 30% for School Type (All Classes)")
plt.ylabel('Number of Students')
plt.tight_layout()
plt.savefig("Top_30_Score_Distribution_All_Classes.png")
plt.show()

# 5. Save the result to an Excel file
grouped.to_excel("Top_30_Score_Distribution.xlsx")
