...
for class_name, class_data in data.groupby('class'):
    # Calculate the number of rows corresponding to the top 10% scores in the current class
    top_10_percent = int(len(class_data) * 0.1)

    # Get the top 10% scores for the current class
    top_scores = class_data.head(top_10_percent)
    ...
    plt.title(f'Distribution of Regions in Top 10% Scores - Class {class_name}')
    plt.axis('equal')  # Equal aspect ratio ensures that pie is drawn as a circle.
    plt.savefig(f'Class_{class_name}_pie_chart_10.png')
    plt.close()
...
plt.title('Distribution of Regions in Top 10% Scores - All Classes Combined')
plt.axis('equal')
plt.savefig('All_Classes_pie_chart_10.png')
plt.close()
...
result_table.to_excel('result_table_10.xlsx', index=False)
